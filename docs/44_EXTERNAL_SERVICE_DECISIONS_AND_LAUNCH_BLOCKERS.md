...
| Email y antiabuso | proveedor, dominio verificado, SPF/DKIM/DMARC, remitente, SMTP y CAPTCHA | completo el 2026-07-23: Resend verificó el dominio; DMARC está publicado; Supabase tiene SMTP Resend y el secreto de Turnstile; Vercel Production tiene el site key público | `DISABLE_PUBLIC_REGISTRATION=true` y `disable_signup=true` hasta publicar términos revisados y probar alta, confirmación y recuperación reales |
