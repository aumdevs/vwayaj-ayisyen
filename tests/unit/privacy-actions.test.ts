...
const mocks = vi.hoisted(() => {
  const roleLimit = vi.fn();
  const roleIn = vi.fn(() => ({ limit: roleLimit }));
  const roleEq = vi.fn(() => ({ in: roleIn }));
...
beforeEach(() => {
  ...
  mocks.roleLimit.mockResolvedValue({ data: [{ role: "admin" }], error: null });
...
describe("privacy administrator completion action", () => {
  it("submits only the validated terminal decision through the protected RPC", async () => {
    ...
    expect(mocks.roleEq).toHaveBeenCalledWith("user_id", "00000000-0000-4000-8000-000000000123");
    expect(mocks.roleIn).toHaveBeenCalledWith("role", ["admin", "super_admin"]);
  ...
  it("rejects an administrator session below AAL2 before reading roles or calling the RPC", async () => {
    mocks.getClaims.mockResolvedValue({
      data: {
        claims: {
          aal: "aal1",
  ...
  it("rejects AAL2 users without an administrator role before calling the RPC", async () => {
    mocks.roleLimit.mockResolvedValue({ data: [], error: null });
  ...
  it("keeps the RPC authorization as an independent second boundary", async () => {
    mocks.rpc.mockResolvedValue({
      data: null,
