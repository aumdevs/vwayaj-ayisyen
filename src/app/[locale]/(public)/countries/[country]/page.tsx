...
import { MobileCountryGuide } from "@/components/public/mobile-country-guide";
...
import { countries, getCountry } from "@/lib/content/catalog";
...
export default async function CountryPage({ params }: CountryPageProps) {
  ...
  if (
    !isLocale(locale) ||
    locale !== "ht" ||
    (countryParam !== "chile" && countryParam !== "brazil")
  )
    notFound();
  ...
  const whatsappValue = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER?.replace(/\D/g, "") ?? "";
  const whatsappNumber = /^\d{8,15}$/.test(whatsappValue) ? whatsappValue : null;

  return (
    <>
      <MobileCountryGuide country={countryParam} whatsappNumber={whatsappNumber} />
      <div className="desktop-country-experience">
